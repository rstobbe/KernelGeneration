...
function [OUTPUT,err] = Inv_KB2DCart_v1b_Func(INPUT)
...
%-------------------------------------------------  
KB = ifftshift(KB); 
V = zerofill_isotropic2D_even_doubles(KB,anlzzf);
...
figure(3); hold on
plot(t,imag(V(:,anlzzf/2+1)),'b');                                 % imaginary component should be ~ zero
plot(t,real(V(:,anlzzf/2+1)),'r');                                 % real component - 'minimal fold-over at edges'
plot(t,squeeze(real(V(anlzzf/2+1,:))),'r'); 
...
V = zerofill_isotropic2D_odd_doubles(ifftshift(V2),ZF);
...
IFprms.V = V;
name = ['IF_KBC2Dw',num2str(W,2),'b',num2str(beta,3),'ss',num2str(SS,3),'zf',num2str(ZF,3)];
