...
ZFinter = str2double(SCRPTipt(strcmp('ZFinter',{SCRPTipt.labelstr})).entrystr); 
...
if rem((W/2),res)
    err.flag = 1;
    err.msg = 'width/2 not a multiple of kernel resolution';
    return
end
if rem((ZFinter/2),res)
    err.flag = 1;
    err.msg = 'ZFinter/2 not a multiple of kernel resolution';
    return
end
...
V = zerofill_isotropic_even_doubles(ifftshift(V2),ZF);
