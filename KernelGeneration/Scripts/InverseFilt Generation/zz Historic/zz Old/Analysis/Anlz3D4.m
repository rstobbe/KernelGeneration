...
V0 = Kzerofill_isotropic(KB1,zf1);
V0 = fftshift(fftn(V0));                              
V0 = V0/max(V0(:));
...
plot(t,real(RepAmpErr(:,zf1/2+1,zf1/2+1)),'b');                                   % imaginary component should be ~ zero
plot([0.5 0.5],[-1 2],'k:');
plot([-0.5 -0.5],[-1 2],'k:');
plot([0.6 0.6],[-1 2],'k:');
plot([-0.6 -0.6],[-1 2],'k:');
...
figure(21)
colormap('jet');
T = real(RepAmpErr(zf1/2+1+(-6:6),zf1/2+1+(-6:6),zf1/2+1+(0)));
h = image(T);
colorbar
