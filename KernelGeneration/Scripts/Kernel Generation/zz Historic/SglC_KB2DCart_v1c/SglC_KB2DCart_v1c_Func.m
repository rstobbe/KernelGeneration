...
function [KRNprms,err] = SglC_KB2DCart_v1c_Func(INPUT,KRNprms)
...
tKern = zeros(length(CSVu),length(CSVu));
N = length(CSVu);
for a = 1:N
    for b = 1:N
        tKern(a,b) = CSVKaiserProf(a)*CSVKaiserProf(b);
    end
...
if strcmp(KRNprms.prec,'singles')
    Kern = zeros((zW/2)*round(1/res)+1,(zW/2)*round(1/res)+1,'single');
elseif strcmp(KRNprms.prec,'doubles')
    Kern = zeros((zW/2)*round(1/res)+1,(zW/2)*round(1/res)+1,'double');
end
N = length(KaiserProf);
for a = 1:N
    for b = 1:N
        Kern(a,b) = KaiserProf(a)*KaiserProf(b);
    end
    Status2('busy',num2str(N-a),2);    
end
Status2('done','',2);
...
figure(1100); hold on; 
plot((1:length(Kern)),squeeze(Kern(1,:))); 
xlim([0 length(Kern)]); 
xlabel('Kernel Size','fontsize',10,'fontweight','bold');

figure(1101); hold on; 
plot((0:length(Kern)-1)*res,squeeze(Kern(1,:))); 
xlim([0 zW/2]); 
xlabel('Kernel Width','fontsize',10,'fontweight','bold');

figure(1102); hold on;
anlzzf = 64000;
KernProf = [KaiserProf zeros(1,anlzzf-2*length(KaiserProf)+1) flipdim(KaiserProf(2:length(KaiserProf)),2)];
...
KRNprms.zW = zW;
KRNprms.Kern = Kern;
name = ['Kern_KBC2Dw',num2str(W,2),'b',num2str(beta,3),'ss',num2str(SS,3)];
