%==================================================
% (v1b) 
%    - allow different resolutions for 2 kernels
%==================================================

function [SCRPTipt,SCRPTGBL,err] = DblC_FSKB_v1b(SCRPTipt,SCRPTGBL)
...
if rem((FS_W/2),FS_res) || rem((KB_W/2),KB_res)
    err.flag = 1;
    err.msg = 'KernWidth/2 not a multiple of KernRes';
    return
end
if rem(1,FS_res) || rem(1,KB_res)
    err.flag = 1;
    err.msg = '1/KernRes is not an integer';
    return
end
if rem(round(1e9*(1/(FS_res*SS)))/1e9,1) || rem(round(1e9*(1/(KB_res*SS)))/1e9,1)
    err.flag = 1;
...
if zWtest > KB_zW
    err.flag = 1;
    err.msg = 'KB Zero-Fill Too Small';
    return
end
...
if KB_zW/KB_res > 1100
    err.flag = 1;
    err.msg = ['KB kernel size: ',num2str(KB_zW/KB_res)];
    %return
end
...
if strcmp(KRNprms.prec,'singles')
    KBKern = zeros((KB_zW/2)*round(1/KB_res)+1,(KB_zW/2)*round(1/KB_res)+1,(KB_zW/2)*round(1/KB_res)+1,'single');
elseif strcmp(KRNprms.prec,'doubles')
    KBKern = zeros((KB_zW/2)*round(1/KB_res)+1,(KB_zW/2)*round(1/KB_res)+1,(KB_zW/2)*round(1/KB_res)+1,'double');
end
N = length(KB_1D);
for a = 1:N
    for b = 1:N
