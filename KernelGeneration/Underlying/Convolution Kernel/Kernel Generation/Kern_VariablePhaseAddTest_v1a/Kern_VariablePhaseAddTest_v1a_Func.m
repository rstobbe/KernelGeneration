...
function [KDES,err] = Kern_VariablePhaseAddTest_v1a_Func(KDES,INPUT)
...
%PhaseMat = ones(LenRelImDomain,LenRelImDomain,LenRelImDomain);
%PhaseMat(LenRelImDomain/2-1:LenRelImDomain/2+1,LenRelImDomain/2-1:LenRelImDomain/2+1,LenRelImDomain/2-1:LenRelImDomain/2+1) = exp(1i*(pi/4));
%PhaseMat(LenRelImDomain/2+1,LenRelImDomain/2+1,LenRelImDomain/2+1) = exp(1i*(pi/3));
PhaseMat = zeros(LenRelImDomain,LenRelImDomain,LenRelImDomain);
%PhaseMat(LenRelImDomain/2+1,LenRelImDomain/2+1,LenRelImDomain/2+1) = 2*pi;
PhaseMat(LenRelImDomain/2+2,LenRelImDomain/2+2,LenRelImDomain/2+2) = 2*pi;
FtPhaseMat = fftshift(fftn(ifftshift(PhaseMat)));
Filt = hamming(LenRelImDomain,'periodic');
FiltMat1 = repmat(Filt,1,LenRelImDomain,LenRelImDomain);
Filt2(1,:) = Filt;
FiltMat2 = repmat(Filt2,LenRelImDomain,1,LenRelImDomain);
Filt3(1,1,:) = Filt;
FiltMat3 = repmat(Filt3,LenRelImDomain,LenRelImDomain,1);
FiltMat = FiltMat1 .* FiltMat2 .* FiltMat3;
FtPhaseMat = FtPhaseMat .* FiltMat;
PhaseMat = fftshift(ifftn(ifftshift(FtPhaseMat)));
PhaseMat = exp(1i.*PhaseMat);
Bot = Cen-W;
Top = Cen+W-1;
